...
@elseif(auth()->user()->email == $company->clients->client_email)
    <tr id="{{ $company->id }}" class="@if($days < 90 && $days > 6) warning-bg @elseif($days < 5) danger-bg @elseif($days > 356) good-bg @endif">
        ...
        <td class="p-3 border border-red-800">
                <button disabled type="submit" class="btn btn-delete"> <i class="icon icon-delete" style="background-image: url({{ asset('img/bin.png') }})"></i> Delete</button>
        </td>
        <td class="p-3 border border-red-800">
                <a disabled href="javascript;" class="btn btn-edit"><i class="icon icon-edit" style="background-image: url({{ asset('img/edit.png') }})"></i> Edit</a>
        </td>
    </tr>
@endif